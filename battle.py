...
from util import LoopController, LoopControllerManager, get_level_from_experience, Popup
...
class CombatLog:  # Lazy init?
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            try:
                if event.button == 4:  # Scroll up
                    self.combat_log_start_index = max(
                        0, self.combat_log_start_index - 1
                    )
                elif event.button == 5:  # Scroll down
                    self.combat_log_start_index = min(
                        len(self.messages) - self.visible_messages,
                        self.combat_log_start_index + 1,
                    )
    ...
    def draw(self):
        ...
        end_index = min(
            self.combat_log_start_index + self.visible_messages, len(self.messages)
        )
...
def add_popup(
    ...
    if font_style:
        font = pygame.font.SysFont(
            font, font_size, bold="bold" in font_style, italic="italic" in font_style
        )
    else:
...
class Battle:
    ...
    def print_battle_menu(self, difficulty, bait):
        ...
        while loop_controller.active:
            ...
            for event in events:
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if event.button == 4:  # Scroll up
                        try:
                            global_log.combat_log_start_index = max(
                                0, global_log.combat_log_start_index - 1
                            )
                        except IndexError:
                            pass
                    elif event.button == 5:  # Scroll down
                        try:
                            global_log.combat_log_start_index = min(
                                len(global_log.messages) - global_log.visible_messages,
                                global_log.combat_log_start_index + 1,
                            )
    ...
    @LogDecorator(log=get_global_log())
    def calculate_drag(self, difficulty, bait, adjustment=0, adjustment_direction=None):
    ...
    @LogDecorator(log=get_global_log())
    def let_the_fish_take_line(self, difficulty, bait, pre_battle_fish_stamina):
        ...
        if fish.stamina <= pre_battle_fish_stamina * 0.2:
            ...
            print(
                f"Your {character.gear[0]['rod'].reel.name} squeals under the pressure."
            )
    ...
    def calculate_fish_stamina_loss(self, difficulty, bait, weight="normal"):
        character = self.character
        fish = self.fish
        if weight == "high":
    ...
    def print_battle_summary(self, difficulty, bait, exp):
        ...
        popup = Popup(manager, messages, buttons, size=(300, 500), debug=True)
    ...
    def end_battle_summary_loop(self):
        loop_controller = LoopControllerManager.get_controller(
            "print_battle_summary"
        )
        loop_controller.active = False
